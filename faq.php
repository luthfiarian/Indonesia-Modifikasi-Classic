...
<head>
    ...
    <meta name="title" content="FAQ | IMC">
    <meta name="description" content="Pertanyaan yang sering diajukan oleh pelanggan kami.">
...
<body>
    ...
    <content class="pt-3.5">
        <!-- FAQ Section -->
        <section id="faq" class="pt-36 pb-40">
            <div class="container">
                <article id="the-article">
                    <div class="max-w-xl mx-auto text-center ">
                        <h2 class="font-bold text-3xl mb-2 text-dark">FAQ</h2>
                        <hr class="w-24 border-b-2 mx-auto mb-3 border-primary rounded-full mt-5">
                        <p class="mb-2">Frequently Asked Questions</p>
                    </div>

                    <?php while ($data_faq = mysqli_fetch_assoc($result_faq)) { ?>
                        <div class="p-4">
                            <div class="item px-20 py-1" x-data="{ isOpen : false } ">
                                <a href="#" class="flex items-center justify-between" @click.prevent="isOpen = true">
                                    <h4 :class="{'text-dark font-medium' : isOpen == true}"><?php echo $data_faq['pertanyaan'] ?></h4>
                                ...
                                <div x-show="isOpen" @click.away="isOpen = false" x-transition:enter="transition ease-out origin-top-left duration-200" x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition origin-top-left ease-in duration-100" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-90" class="mt-3 transition duration-300 ease-in-out " :class="{'text-gray-600' : isOpen == true}">
                                    <p><?php echo $data_faq['jawaban'] ?></p>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                ...
                <!-- <div x-data="scrollHandler(document.getElementById('the-article'))" x-cloak aria-hidden="true" @scroll.window="calculateHeight(window.scrollY)" class="fixed h-screen w-1 hover:bg-gray-200 top-0 left-0 bg-gray-300">
                    <div :style="`max-height:${height}%`" class="h-full bg-g reen-400"></div>
                </div>
